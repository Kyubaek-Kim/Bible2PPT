...
- **타이포/레이아웃 커스터마이징** — 화면 비율, 글꼴·크기·굵기, 그리고 제목·구절·본문 상자의
  **위치(안쪽 드래그)와 크기(모서리 드래그)** 를 조정해 저장. 현재 배경이 흐리게 미리보기로 깔리고,
  변경 영역은 실제 PPT 장표에 그대로 반영된다.
...
2. 화면은 세로형(휴대폰 크기) 레이아웃입니다. 위에서부터:
   ...
   - **화면 설정**: 화면 비율(16:9 / 4:3 / A4), 글자체(미리보기 제공), 본문 글자크기, 본문 굵게.
     하단 **화면 구성 커스터마이징**에서 제목·구절·본문 상자를 드래그로 **재배치**하고 모서리를 끌어
     **크기 조절**하며(현재 배경이 흐리게 미리보기됨), 제목·구절의 크기/굵기/활성화를 지정한 뒤 저장할
     수 있습니다(초기화로 기본 배치 복원). 조절한 영역은 실제 PPT 장표에 그대로 반영됩니다.
...
번들 폰트: **나눔스퀘어 그룹**(Light/Regular/Bold(기본)/ExtraBold)과 **나눔고딕 그룹**(Light/Regular/Bold/ExtraBold) — 모두 SIL Open Font License (`data/fonts/OFL.txt`). 글꼴 드롭다운에는 이 번들 글꼴과 함께 Windows 기본 글꼴(**맑은 고딕·굴림·돋움·바탕·궁서**)이 표시됩니다. 본문 굵기는 체크박스로 켜고 끌 수 있습니다.
